...
$$
x_{t+1} = \mathrm{argmin}_x \{ \mu_{t}(x) - \beta_t \sigma_{t}(x) \}
$$
...
$$
\mu(x_*) = K(X_*, X) \left[ K(X, X) + \sigma_n^2 I_n\right]^{-1} y
$$
...
<img class="col three" src="{{ site.baseurl }}/assets/img/objective_fun.png">
...
First we make a function get the Hessian of an arbitrary function:
 ...
 {% highlight python %}
def hessian(fun):
    return jit(jacfwd(jacrev(fun))) {% endhighlight %}
...
def solve_constrained(x0, xs, ys, length_scale, beta, A):
    ...
    solution) and augmented hessian and grad functions. """
    x = x0
    t = 10
...
We see that the vast majority of the samples are at the actual minimum point,
